...
require([
  ...
  var title = apiProject.title
    ? apiProject.title
    : 'Documentation: ' + apiProject.name + ' - ' + apiProject.version;
  ...
  function initDynamic() {
    ...
    $('.json-schema').click(function (e) {
      ...
      var indx = api.findIndex(function (entry) {
        return entry.name === articleName;
      });
      ...
      jsonSchema =
        '<pre class="prettyprint language-json" data-type="json"><code>' +
        syntaxHighlight(jsonSchema, false) +
        '</code></pre>';
      $('#jsonModal .modal-title').html(
        articleName.split('_').join(' ') + ' - ' + param + ' ' + group
      );
    ...
    $('.modal-clipboard').on('click', function () {
      ...
      var copySuccess = copyToClipboard(
        code,
  ...
  function expandDot(dots, obj, type) {
    ...
    if (obj[par]) {
      if (obj[par].type && obj[par].type === 'array') {
        obj[par]['items'] = expandDot(
          rest,
          obj[par]['items'] ? obj[par]['items'] : {},
          type
        );
      } else {
        obj[par]['properties'] = expandDot(
          rest,
          obj[par]['properties'] ? obj[par]['properties'] : {},
          type
        );
      }
